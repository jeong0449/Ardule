...
SCRIPT_NAME="adc-patternlab.py"; VERSION="260812a"; VERSION_TEXT=f"{SCRIPT_NAME} {VERSION}"
...
def grid_omitted_event_ids(block, subdiv: str) -> Set[int]:
    ...
    ORN candidates (ghosts and flam grace notes) and notes outside the selected
    SLOT_MAP are deliberately excluded from this warning class.
    """
    ...
    for event in block.events:
        if event.vel<=GHOST_CANDIDATE_MAX_VELOCITY:
            continue
...
function updateRawDeviation(card, selected){{
  ...
  card.querySelectorAll('.raw-event[data-tick-offset]').forEach(el=>{{
    ...
    if(el.classList.contains('rawhit')){{
      const offGrid=Math.abs(offset-nearest)>1e-9;
      const ordinary=!el.classList.contains('ornnote')&&!el.classList.contains('unknown');
      ...
      if(title){{
        ...
        title.textContent=title.dataset.baseTitle+(omitted?`; GRID omitted (${{subdiv}}): off-grid by ${{deltaText}} tick(s); nearest grid ${{nearestText}}`:'');
      }}
    }}
  }});
