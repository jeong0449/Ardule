...
SCRIPT_NAME="adc-patternlab.py"; VERSION="260908b"; VERSION_TEXT=f"{SCRIPT_NAME} {VERSION}"
...
.genre-modal-backdrop{{position:fixed;inset:0;z-index:5000;display:flex;align-items:center;justify-content:center;background:rgba(0,0,0,.48)}}.genre-modal-backdrop[hidden]{{display:none}}.genre-modal{{width:min(440px,calc(100vw - 32px));padding:18px;border:1px solid var(--line);border-radius:12px;background:var(--panel);box-shadow:0 18px 48px rgba(0,0,0,.28)}}.genre-modal h2{{margin:0 0 7px;font-size:18px}}.genre-modal p{{margin:0 0 12px;color:var(--muted);font-size:12px;line-height:1.5}}.genre-modal label{{display:grid;grid-template-columns:auto 1fr;gap:8px;align-items:center;font-size:12px;font-weight:700}}.genre-modal select,.genre-modal input{{width:100%;padding:7px;border:1px solid var(--line);border-radius:7px;background:var(--panel);color:var(--ink);text-transform:uppercase}}.genre-modal-hint{{margin-top:8px!important;font-size:10.5px!important}}.csv-drop-zone{{margin-top:12px;padding:14px 10px;border:2px dashed var(--line);border-radius:9px;text-align:center;color:var(--muted);font-size:11px;font-weight:750;cursor:pointer;background:var(--bg)}}.csv-drop-zone.dragover{{border-color:var(--slot);color:var(--slot);background:var(--panel)}}.csv-load-status{{min-height:16px;margin:7px 0 0!important;font-size:10.5px!important}}.csv-load-status.error{{color:#dc2626!important}}.genre-modal-actions{{display:flex;justify-content:flex-end;gap:8px;margin-top:14px}}.number-gap-title{{fill:var(--warn)!important}}
...
function parsePatternLabCsv(text){{
  ...
  const header=rows.shift().map(value=>String(value||'').trim().toUpperCase());
  const required=['START_BAR','END_BAR','NAME','EXPORT','GENRE','SUBDIV','ORN'];
  required.forEach(name=>{{if(!header.includes(name))throw new Error(`Missing column: ${{name}}`);}});
...
function originalPanels(){{
  return [...s.querySelectorAll('.pattern-controls')];
}}
function restorePatternLabCsv(records){{
  const panels=new Map(originalPanels().map(panel=>[
    `${{panel.dataset.startBar}}:${{panel.dataset.endBar}}`,panel
  ]));
  let restored=0,unmatched=0;
  records.forEach(record=>{{
    const panel=panels.get(`${{record.START_BAR}}:${{record.END_BAR}}`);
    if(!panel){{unmatched++;return;}}
    restored++;
    const exp=panel.querySelector('.export-check');
    const genre=panel.querySelector('.genre-select');
    const subdivision=panel.querySelector('.subdivision-select');
    const orn=panel.querySelector('.orn-check');
    const number=panel.querySelector('.start-number');
    if(exp&&!exp.disabled)exp.checked=/^(YES|TRUE|1)$/i.test(record.EXPORT);
    if(genre&&!genre.disabled&&record.GENRE)setGenreCode(genre,record.GENRE);
    if(subdivision&&!subdivision.disabled&&['16','32','8T','16T'].includes(record.SUBDIV)){{
      subdivision.value=record.SUBDIV;applySubdivision(panel);
    }}
    if(orn&&!orn.disabled)orn.checked=/^(YES|TRUE|1)$/i.test(record.ORN);
    if(number&&!number.disabled){{
      number.value='';delete number.dataset.auto;number.classList.remove('invalid');
      // CSV NAME is the authoritative saved pattern number.  Restore the
      // trailing numeric component whether or not EXPORT is currently checked.
      // (EXPORT and numbering are separate pieces of PatternLab state.)
      const nameParts=String(record.NAME||'').split('_');
      const tail=nameParts.length?nameParts[nameParts.length-1]:'';
      if(/^[0-9]{{1,4}}$/.test(tail)){{
        number.value=String(Number(tail)).padStart(4,'0');
        delete number.dataset.auto;  // keep the restored value as a manual anchor
      }}
    }}
  }});
...
async function loadPatternLabCsvFile(file){{
  if(!file)return false;
  const status=document.getElementById('csv-load-status');
  try{{
    const records=parsePatternLabCsv(await file.text());
    const result=restorePatternLabCsv(records);
    if(!result.restored)throw new Error('No CSV rows matched this report by START_BAR / END_BAR.');
    if(status){{
      status.classList.remove('error');
      status.textContent=`Restored ${{result.restored}} row(s) from ${{file.name}}`+
    ...
    if(status){{status.classList.add('error');status.textContent=`CSV load failed: ${{error.message||error}}`;}}
    else alert(`CSV load failed: ${{error.message||error}}`);
    return false;
  }}
}}
function setupCsvImport(){{
  const picker=document.getElementById('patternlab-csv-file');
  const drop=document.getElementById('csv-drop-zone');
  const headerButton=document.getElementById('load-csv');
  if(!picker)return;
  const choose=()=>picker.click();
  if(headerButton)headerButton.addEventListener('click',choose);
  if(drop){{
    ...
    ['dragenter','dragover'].forEach(type=>drop.addEventListener(type,event=>{{
      event.preventDefault();event.stopPropagation();drop.classList.add('dragover');
    }}));
    ['dragleave','drop'].forEach(type=>drop.addEventListener(type,event=>{{
      event.preventDefault();event.stopPropagation();drop.classList.remove('dragover');
    }}));
    drop.addEventListener('drop',event=>{{
      const file=[...(event.dataTransfer?.files||[])].find(item=>item.name.toLowerCase().endsWith('.csv'));
      if(file)void loadPatternLabCsvFile(file);
    }});
  }}
  picker.addEventListener('change',()=>{{
    const file=picker.files?.[0];if(file)void loadPatternLabCsvFile(file);
    picker.value='';
  }});
}}
