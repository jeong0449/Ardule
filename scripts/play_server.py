...
<title>ADX Drum Player</title><style>
...
<footer>Read-only browser. ADP is preferred when same-basename ADT and ADP both exist. Same-basename ORN is applied automatically.</footer>
</section></main><script>
(()=>{
...
let mode='midi',currentId=null,parentId=null,lastData=null,genre='ALL',descending=false,playingId=null,playingName='',playPaused=false,playDuration=0,playStartedAt=0,elapsedBeforePause=0,playAnimation=null,detailId=null,detailKind=null;
...
function setMode(next){if(mode===next)return;mode=next;genre='ALL';detailId=detailKind=null;modeMidi.classList.toggle('active',mode==='midi');modePattern.classList.toggle('active',mode==='pattern');modeHint.textContent=mode==='midi'?'Standard MIDI playback':'Looping ADT/ADP pattern playback';browse(currentId||'start')}
...
function render(data){lastData=data;currentId=data.current_id??null;parentId=data.parent_id??null;locationBox.textContent=data.display_path||'Computer';locationBox.title=locationBox.textContent;upButton.disabled=!parentId;renderFilters(data);const folders=data.folders||[],items=rowsFor(data);if(!folders.length&&!items.length){content.innerHTML='<div class="empty">No matching files here.</div>';status.textContent='Empty';return}let rows='';for(const f of folders)rows+=`<tr class="entry folder" data-id="${esc(f.id)}"><td><span class="folder-name">📁 ${esc(f.name)}</span></td><td class="kind">Folder</td><td></td><td></td></tr>`;if(mode==='midi'){for(const f of items){const active=f.id===playingId;rows+=`<tr><td><span class="file-name">♪ ${esc(f.name)}</span></td><td class="kind">MIDI</td><td class="num">${duration(f.duration_seconds)}</td><td class="num">${bytes(f.size)} &nbsp; <button class="primary play" data-id="${esc(f.id)}">${active?(playPaused?'▶ Resume':'⏸ Pause'):'▶ Play'}</button>${active?' <button class="midi-stop">■ Stop</button>':''}</td></tr>`;if(active)rows+=`<tr class="playback-row"><td colspan="4"><div class="inline-player"><div class="midi-status-line"><span class="midi-status-state">${playPaused?'Paused':'Playing'}: ${esc(playingName)}</span><span class="midi-status-time"></span></div><div class="midi-progress"><span></span></div></div></td></tr>`}content.innerHTML=`<table><thead><tr><th>Name</th><th>Type</th><th class="num">Duration</th><th class="num">Size / Action</th></tr></thead><tbody>${rows}</tbody></table>`;if(playingId)updateMidiProgress()}else{for(const f of items){rows+=`<tr class="${f.id===playingId?'playing-row':''}"><td><span class="file-name">▦ ${esc(f.name)}</span></td><td class="kind">${esc(f.type)}</td><td class="ornflag ${f.has_orn?'yes':''}">${f.has_orn?'ORN ●':'—'}</td><td class="num"><button class="primary play" data-id="${esc(f.id)}">${f.id===playingId?'■ Stop':'▶ Play'}</button> <button class="showpat" data-id="${esc(f.id)}">${detailId===f.id&&detailKind==='pattern'?'Hide Pattern':'Show Pattern'}</button>${f.has_orn?` <button class="showorn" data-id="${esc(f.id)}">${detailId===f.id&&detailKind==='orn'?'Hide ORN':'Show ORN'}</button>`:''}</td></tr>`;if(detailId===f.id)rows+=`<tr class="detail-row"><td colspan="4"><div id="detailBox" class="detail-box">Loading…</div></td></tr>`}content.innerHTML=`<table><thead><tr><th>Name</th><th>Type</th><th>ORN</th><th class="num">Action</th></tr></thead><tbody>${rows}</tbody></table>`;if(detailId)loadDetail(detailId,detailKind)}
content.querySelectorAll('.folder').forEach(r=>r.onclick=()=>browse(r.dataset.id));content.querySelectorAll('.play').forEach(b=>b.onclick=e=>{e.stopPropagation();togglePlayback(b.dataset.id)});content.querySelectorAll('.midi-stop').forEach(b=>b.onclick=e=>{e.stopPropagation();stopPlayback()});content.querySelectorAll('.showpat').forEach(b=>b.onclick=()=>toggleDetail(b.dataset.id,'pattern'));content.querySelectorAll('.showorn').forEach(b=>b.onclick=()=>toggleDetail(b.dataset.id,'orn'));renderGlobalTransport();status.textContent=mode==='midi'?`${folders.length} folder(s) · ${items.length} MIDI`:`${folders.length} folder(s) · ${items.length} pattern(s)`}
