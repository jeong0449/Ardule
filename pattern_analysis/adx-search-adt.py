...
"""
ADX Drum / Ardule
External ADT similarity search
...
# Authoritative ADX build/search modules. These remain sibling scripts, as in
# adx_search_adt_v0.2a.py; the CLI refactor must not remove these paths.
...
def locate_slot_maps(explicit: Path | None) -> Path:
    ...
    raise FileNotFoundError(
        "cannot locate slot_map_definitions.json; place it beside the script or in ../lib, "
        "or use --slot-maps"
    )


def corpus_paths(corpus_index_dir: Path) -> Dict[str, Path]:
    root = corpus_index_dir.expanduser().resolve()
    return {
        "projection": root / "search_projection.jsonl",
        "occurrences": root / "occurrences.tsv",
        "canonical": root / "canonical_patterns.jsonl",
    }
...
def write_results(
    ...
    lines = [
        "ADX External ADT Similarity Search",
        "=" * 72,
...
def write_html_report(report_path, adt_path, structure, results_by_bar, pid_sources, canonical_by_id, projection_by_id, projection_mod, slot_map_path, top, alpha):
    ...
    css='''
:root{--bg:#f5f6f8;--panel:#fff;--ink:#20242a;--muted:#68707a;--line:#dde1e6;--accent:#5488c7;--query:#eef5ff;--h1:#dbeafe;--h2:#93c5fd;--h3:#3b82f6;--h4:#1e3a8a}
@media(prefers-color-scheme:dark){:root{--bg:#11151a;--panel:#1a2027;--ink:#e6edf3;--muted:#9da9b5;--line:#303843;--accent:#78a9e6;--query:#202b3a;--h1:#23395d;--h2:#2f6fab;--h3:#58a6ff;--h4:#b6d8ff}}
*{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--ink);font:14px/1.45 system-ui,-apple-system,Segoe UI,Arial,sans-serif}main{max-width:1180px;margin:0 auto;padding:0 28px 28px}.report-header{position:sticky;top:0;z-index:100;background:var(--bg);padding:18px 0 14px;border-bottom:1px solid var(--line);box-shadow:0 3px 8px #0000000c}.report-header h1{margin:0 0 4px;font-size:28px}.sub,.meta{color:var(--muted)}.sub{font-size:13px}.view-toolbar{display:flex;align-items:center;gap:7px;margin:14px 0 16px;padding:9px 11px;border:1px solid var(--line);border-radius:10px;background:var(--panel)}.view-toolbar strong{font-size:11px;color:var(--muted);text-transform:uppercase;letter-spacing:.04em}.view-toolbar button.active{background:var(--accent);border-color:var(--accent);color:#fff}.view-toolbar .legend{margin-left:auto;font-size:10px;color:var(--muted);white-space:nowrap}.query-area{margin-bottom:18px}.results-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(min(100%,360px),1fr));gap:14px;align-items:start}.pattern-card{min-width:0;padding:18px;border:1px solid var(--line);border-radius:12px;background:var(--panel);box-shadow:0 1px 3px #00000008}.query-card{border:2px solid var(--accent);background:var(--query)}.card-copy{margin-bottom:10px}.card-title-row{display:flex;align-items:flex-start;justify-content:space-between;gap:16px;min-width:0}.card-title-row h2{margin:0 0 4px;font-size:18px;line-height:1.2;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.score{display:flex;align-items:center;gap:6px;flex:0 0 auto;font-variant-numeric:tabular-nums}.score b{font-size:18px}.score span{padding:4px 7px;border:1px solid var(--line);border-radius:999px;font-size:10px;font-weight:750;white-space:nowrap}.provenance,.identity,.metrics{overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:11px;line-height:1.45}.grid-wrap{overflow-x:auto;overflow-y:hidden;margin:12px 0 0;background:#fafafa;border:1px solid var(--line);border-radius:8px;padding:9px 10px}.query-card .grid-wrap{background:var(--query)}.pattern-grid{border-collapse:collapse;width:100%;table-layout:fixed}.pattern-grid th{width:42px;padding:1px 4px;border:1px solid var(--line);background:transparent;font:600 9px/1 ui-monospace,SFMono-Regular,Consolas,monospace;text-align:right;color:var(--muted)}.pattern-grid td{height:17px;padding:0;text-align:center;border:1px solid var(--line);font:700 8px/1 ui-monospace,Consolas,monospace}.pattern-grid td.beat{border-left:2px solid #9ca4ad}.pattern-grid td.vweak,.pattern-grid td.weak{background:var(--h1)}.pattern-grid td.medium{background:var(--h2)}.pattern-grid td.strong{background:var(--h3);color:#fff}.pattern-grid td.accent{background:var(--h4);color:#fff}.actions{display:flex;gap:6px;margin-top:10px}button{margin:0;padding:5px 9px;border:1px solid var(--line);border-radius:6px;background:var(--panel);color:var(--ink);font-size:10px;font-weight:750;cursor:pointer}button.play{border-color:var(--accent);color:var(--accent)}#service.online{color:#16a34a}#service.offline{color:#dc2626}.diff-missing{outline:2px solid #dc2626;outline-offset:-2px;color:#dc2626!important;background:transparent!important}.diff-extra{outline:2px solid #2563eb;outline-offset:-2px}.diff-shifted{outline:2px dashed #d97706;outline-offset:-2px}.diff-strength{box-shadow:inset 0 0 0 2px #a855f7}.section-title{font-size:17px;margin:20px 0 9px}.result-section{margin-bottom:18px}.footer-note{color:var(--muted);padding:10px 0 24px;font-size:11px}@media(max-width:720px){main{padding:0 12px 18px}.report-header h1{font-size:22px}.view-toolbar{align-items:flex-start;flex-wrap:wrap}.view-toolbar .legend{margin-left:0;white-space:normal;width:100%}.results-grid{grid-template-columns:1fr}}
'''
    ...
    doc='<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">'+f'<title>{html.escape(adt_path.name)} — ADX External ADT Search</title><style>{css}</style></head><body><main><header class="report-header"><h1>ADT → Corpus Similarity Search</h1><div class="sub">{html.escape(adt_path.name)} · {html.escape(structure)} · Top {top} · combined {(1-alpha):.2f} rhythm + {alpha:.2f} strength · <span id="service">checking play_server…</span></div><div class="view-toolbar"><strong>View</strong><button id="nativeView" class="active">Native</button><button id="familyView">Family / Diff</button><span class="legend">Family diff: red × query-only · blue candidate-only · orange shifted ±1 · purple strength</span></div></header><div class="query-area">{"".join(query_cards)}</div>{"".join(result_sections)}<div class="footer-note">{html.escape(VERSION_TEXT)} · Query is not added to the corpus index.</div></main><script>{js}</script></body></html>'
...
def parse_args(argv=None):
    ...
    p.add_argument("--top", type=int, default=20, help="Top N corpus matches per query bar (default: 20)")
    ...
    p.add_argument(
        ...
        help="Directory for optional TSV/TXT/HTML outputs (default: current directory)",
    )
    p.add_argument("--write", action="store_true", help="Write TSV and TXT result files")
    p.add_argument(
        "--no-report", action="store_true",
        help="Do not write the interactive HTML report (HTML is written by default)",
    )
    p.add_argument(
...
def main(argv=None):
    ...
    corpus_index_dir = args.corpus_index_dir.expanduser().resolve()
    if not corpus_index_dir.is_dir():
        print(f"ERROR: corpus index directory not found: {corpus_index_dir}", file=sys.stderr)
        return 2
    cpaths = corpus_paths(corpus_index_dir)
    if not cpaths["projection"].is_file():
        print(f"ERROR: search projection not found: {cpaths['projection']}", file=sys.stderr)
        return 2
    if not args.no_report and not cpaths["canonical"].is_file():
        print(f"ERROR: canonical patterns not found: {cpaths['canonical']}", file=sys.stderr)
        return 2
    ...
    try:
        ...
        if not args.no_report:
            canonical_by_id = read_jsonl_by_pattern(cpaths["canonical"])
            ...
            write_html_report(report_path, adt_path, structure, results_by_bar, pid_sources, canonical_by_id, projection_by_id, projection_mod, slot_map_path, args.top, args.alpha)
