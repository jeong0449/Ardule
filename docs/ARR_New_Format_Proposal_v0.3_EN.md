...
**Document Version 0.3 --- 2026-08-13**
...
> is currently being tested in **Ardule Drum Studio v0.4.2**.
...
5.  All `[SECTION]` definitions must appear before `[CHAIN]`.
6.  An ARR file must contain exactly one `[CHAIN]`, and `[CHAIN]` is the
    final structural block of the file.
7.  No `[SECTION]` definition may appear after `[CHAIN]`.
8.  Pattern references use the ADT `NAME`.
9.  `@A` / `@B` select a bar.
10. `:n-m` specifies an inclusive beat range within the selected bar.
11. `*N` specifies the repetition count of the element.
12. `REST:nBAR` / `REST:nBEAT` insert silence.
13. Count-in accepts only `COUNTIN:1BAR` or `COUNTIN:1/2BAR`.
14. Count-in sound is fixed to Closed Hi-Hat.
15. `[CHAIN]` may mix section references and pattern references.
16. A reference to a nonexistent pattern or section is an error.
17. An invalid bar or beat range is reported as an error before
    playback.
18. Required pattern dependencies are extracted automatically by parsing
    the complete ARR.
...
## 22. Planned Additions Before ARR v0.1 Finalization

The following small syntax extensions are planned for evaluation and
implementation before the ARR v0.1 specification is finalized. They are
**not yet part of the normative syntax described above**.

### Multiple elements on one line

Multiple chain elements will be allowed on the same line using comma
(`,`) as the separator.

``` text
RCK_0001, RCK_0002, RCK_0003
```

Newline and comma will both act as sequence separators. Whitespace
before or after a comma will be ignored. The recommended canonical style
is a comma followed by one space.

Semicolon (`;`) remains reserved for comments and will not be used as an
element separator.

### Immediate tempo change

An immediate tempo-change element is planned:

``` text
TEMPO:128
```

`TEMPO:n` will change the playback tempo from that point onward and
remain in effect until another `TEMPO:n` element is encountered. The
header `TEMPO=` value remains the initial arrangement tempo.

When Ardule Drum Studio BPM Override is enabled, the override value is
expected to take precedence over both the header tempo and
in-arrangement `TEMPO:n` changes.

Tempo ramps, accelerando, and ritardando are not included in this
planned extension.

------------------------------------------------------------------------

## 23. Features Intentionally Deferred

This proposal does not include:

-   gradual tempo automation / tempo ramps
-   scoped or automatically restored per-section tempo changes
