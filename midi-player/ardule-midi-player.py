...
SCRIPT_NAME = "ardule-midi-player.py"
VERSION = "0.1"
...
DEFAULT_FLUIDSYNTH = APP_DIR / "fluidsynth" / "fluidsynth.exe"
DEFAULT_SOUNDFONT = APP_DIR / "soundfont" / "GeneralUser-GS.sf2"


NO_REPORT_HTML = r"""<!doctype html>
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Ardule MIDI Player</title><style>
...
<header><h1>Ardule MIDI Player <span class="status">v0.1</span></h1><p>Browse and play Standard MIDI Files with a Channel 10 drum-roll view.</p></header>
<div class="modebar"><b>Playback</b><button id="mixFull" class="active">Full Mix</button><button id="mixSolo">Drums Solo</button><button id="mixMute">Drums Mute</button><span class="spacer"></span><span id="modeHint" class="status">All MIDI channels</span></div>
...
<footer>Read-only MIDI browser · Ardule MIDI Player · CH10 drum visualization</footer>
</section></main><script>
(()=>{
...
const refreshButton=$('refresh'),stopButton=$('stop'),upButton=$('up'),rootsButton=$('roots'),mixFull=$('mixFull'),mixSolo=$('mixSolo'),mixMute=$('mixMute'),modeHint=$('modeHint');
let mode='midi',mixMode='full',currentId=null,parentId=null,lastData=null,genre='ALL',descending=false,playingId=null,playingName='',playPaused=false,playDuration=0,playStartedAt=0,elapsedBeforePause=0,playAnimation=null,detailId=null,detailKind=null,midiDetailId=null,midiRollData=null,lastFollowSystem=null;
...
function renderFilters(data){filters.style.display='none';filters.innerHTML=''}
function rowsFor(data){return data.files||[]}
...
function setMixMode(next){mixMode=next;mixFull.classList.toggle('active',next==='full');mixSolo.classList.toggle('active',next==='solo');mixMute.classList.toggle('active',next==='mute');modeHint.textContent=next==='full'?'All MIDI channels':next==='solo'?'Channel 10 only':'All except Channel 10';if(playingId){const id=playingId;playingId=null;togglePlayback(id)}}
refreshButton.onclick=()=>browse(currentId);upButton.onclick=()=>parentId&&browse(parentId);rootsButton.onclick=()=>browse(null);stopButton.onclick=stopPlayback;mixFull.onclick=()=>setMixMode('full');mixSolo.onclick=()=>setMixMode('solo');mixMute.onclick=()=>setMixMode('mute');browse('start');
})();</script></body></html>"""
...
def make_channel_filtered_midi(path: Path, mode: str) -> Path:
    """Create a temporary SMF for CH10 solo/mute while preserving timing."""
    if mode == 'full':
        return path
    if mode not in {'solo','mute'}:
        raise ValueError('invalid playback mode')
    if MidiFile is None:
        raise RuntimeError('mido is required for channel filtering')
    src = MidiFile(path)
    out = MidiFile(type=src.type, ticks_per_beat=src.ticks_per_beat, charset=getattr(src, 'charset', 'latin1'))
    for track in src.tracks:
        new_track = MidiTrack()
        pending = 0
        for msg in track:
            pending += int(msg.time)
            is_channel = hasattr(msg, 'channel')
            is_drum = is_channel and int(msg.channel) == 9
            keep = (not is_channel) or (is_drum if mode == 'solo' else not is_drum)
            if keep:
                cp = msg.copy(time=pending)
                new_track.append(cp)
                pending = 0
        if pending:
            new_track.append(MetaMessage('end_of_track', time=pending))
        elif not new_track or new_track[-1].type != 'end_of_track':
            new_track.append(MetaMessage('end_of_track', time=0))
        out.tracks.append(new_track)
    fd, name = tempfile.mkstemp(prefix=f'midiscape_{mode}_', suffix='.mid')
    os.close(fd)
    temp_path = Path(name)
    out.save(temp_path)
    return temp_path

def make_handler(player: PlayerState, directory: Path, browser: FileBrowser, report_selected: bool=False, pattern_cfg: dict|None=None):
    class Handler(SimpleHTTPRequestHandler):
        ...
        def do_GET(self):
            ...
            if path=='/api/browse':
                try:
                    q=parse_qs(urlparse(self.path).query); item=q.get('id',[None])[0]
                    self._send_json(200,browser.browse(item,'midi'))
        ...
        def do_POST(self):
            ...
            if path=='/play-file':
                try:
                    payload=self._json_payload(); file_id=payload.get('id') if isinstance(payload,dict) else None; mode=(payload.get('mode') if isinstance(payload,dict) else 'full') or 'full'; midi_path=browser.resolve_midi(file_id); play_path=make_channel_filtered_midi(midi_path,mode); player.play_path(play_path,loop=False,temp_owned=(play_path!=midi_path)); self._send_json(200,{'status':'playing','id':file_id,'name':midi_path.name,'duration_seconds':midi_duration_seconds(midi_path),'mode':mode})
        ...
        def log_message(self,fmt,*args): print(f'[Ardule MIDI Player] {self.address_string()} - {fmt % args}')
    return Handler
...
def resolve_fluidsynth(explicit: Path | None, parser: argparse.ArgumentParser) -> tuple[Path, str]:
    ...
    if fallback.is_file():
        return fallback.resolve(), "bundled"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Ardule MIDI Player - web-based MIDI player with CH10 drum-roll view")
    parser.add_argument("--version", action="version", version=VERSION_TEXT)
    parser.add_argument("--fluidsynth", type=existing_file, default=None, help=f"override FluidSynth path; bundled default: {DEFAULT_FLUIDSYNTH}")
    parser.add_argument("--sf2", type=existing_file, default=None, help=f"override SoundFont path; bundled default: {DEFAULT_SOUNDFONT}")
    ...
    handler = make_handler(player, directory, browser)
    server = ThreadingHTTPServer((HOST, args.port), handler)
    base_url = f"http://{HOST}:{args.port}/"

    print(f"Ardule MIDI Player ({VERSION_TEXT})")
    print(f"  URL        : {base_url}")
    print(f"  MIDI Home  : {directory}")
